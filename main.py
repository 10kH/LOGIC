...
def parse_args():
    ...
    # parser.add_argument(
    #     '--with_reasoning_generation',
    #     action='store_true',
    #     help="Adding reasoning generation data.")
    ...
    parser.add_argument(
        '--with_new_topic',
        action='store_true',
        help="Adding with_new_topic."
    )

    parser.add_argument(
        '--with_topic_str_chatgpt',
        action='store_true',
        help="Adding with_topic_str_chatgpt."
    )
    ...
    parser.add_argument(
        '--train_path',
        default="data/VAST_reasoing_long_and_short.csv",
        type=str
    )

    parser.add_argument(
        '--dev_path',
        default="data/vast_dev.csv",
        type=str
    )

    parser.add_argument(
        '--test_path',
        default="data/vast_test.csv",
        type=str
    )

    parser.add_argument(
        '--output_dir',
        default="../save_dir/add_reason_4500_ts",
        type=str
    )
...
def train(model, train_dataloader, train_dataloader_topic_prediction, train_dataloader_unlikelihood, train_dataloader_topic_unlikelihood, train_dataloader_shuffling, train_dataloader_short, train_dataloader_long , dev_dataloader, device, tokenizer, args):
    weight_decay = 0
    ...
    if train_dataloader_shuffling is not None:
        train_dataloader_shuffling = accelerator.prepare(train_dataloader_shuffling)
        shuffling_iter = iter(train_dataloader_shuffling)
    ...
    for epoch in range(1, args.epoch+1):
        ...
        for step, batch in tqdm(enumerate(train_dataloader),
            ...
            # if train_dataloader_topic_unlikelihood is not None:
            #     try:
            #         topic_unlikelihood_batch = next(topic_unlikelihood_iter)
            #     except StopIteration:
            #         topic_unlikelihood_iter = iter(train_dataloader_topic_unlikelihood)
            #         topic_unlikelihood_batch = next(topic_unlikelihood_iter)
            #
            #     input_ids = topic_unlikelihood_batch['input_ids']
            #     attention_mask = topic_unlikelihood_batch['attention_mask']
            #     negative_loss_mask = topic_unlikelihood_batch['negative_loss_mask']
            #     labels = topic_unlikelihood_batch['labels']
            #
            #     labels[labels == args.pad_token_id] = -100
            #
            #     outputs = model(input_ids=input_ids,
            ...
            if train_dataloader_shuffling is not None:
                try:
                    shuffling_batch = next(shuffling_iter)
                except StopIteration:
                    shuffling_iter = iter(train_dataloader_shuffling)
                    shuffling_batch = next(shuffling_iter)

                input_ids = shuffling_batch['input_ids']
                attention_mask = shuffling_batch['attention_mask']
                labels = shuffling_batch['labels']
...
if __name__ == '__main__':
    ...
    data_dev = pd.read_csv(args.dev_path)
    ...
    train_dataset = VastDataset(data=data, tokenizer=tokenizer, is_train=True, train_mode="stance_prediction", args=args)
    ...
    if args.with_topic_prediction:
        train_dataset_topic_prediction = VastDataset(data=data, tokenizer=tokenizer, is_train=True, train_mode="topic_prediction",
                                    args=args)
        train_dataloader_topic_prediction = DataLoader(train_dataset_topic_prediction, batch_size=args.batch_size, shuffle=True)

    if args.with_unlikelihood_training:
        train_dataset_unlikelihood = VastDataset(data=data, tokenizer=tokenizer, is_train=True,
        ...
        # train_dataset_topic_unlikelihood = VastDataset(data=data, tokenizer=tokenizer, is_train=True,
        #                                                train_mode="topic_unlikelihood_training",
        #                                                args=args)
        # train_dataloader_topic_unlikelihood = DataLoader(train_dataset_topic_unlikelihood, batch_size=args.batch_size,
        #                                                  shuffle=True)

    if args.with_shuffling:
        train_dataset_shuffling = VastDataset(data=data, tokenizer=tokenizer, is_train=True,
                                                     train_mode="shuffling",
                                                     args=args)
        train_dataloader_shuffling = DataLoader(train_dataset_shuffling, batch_size=args.batch_size,
                                                       shuffle=True)

    if args.is_short:
        train_dataset_short = VastReasoningDataset(data, tokenizer=tokenizer, args=args, is_long=False)
        train_dataloader_short = DataLoader(train_dataset_short, batch_size=args.batch_size, shuffle=True)

    if args.is_long:
        train_dataset_long = VastReasoningDataset(data, tokenizer=tokenizer, args=args, is_long=True)
        train_dataloader_long = DataLoader(train_dataset_long, batch_size=args.batch_size, shuffle=True)

    dev_dataset = VastDataset(data=data_dev, tokenizer=tokenizer,args=args, is_train=False, train_mode="stance_prediction")
    dev_dataloader = DataLoader(dev_dataset, batch_size=args.batch_size * 2, shuffle=False)

    test_dataset = VastDataset(data=data_test, tokenizer=tokenizer, args=args, is_train=False, train_mode="stance_prediction")
    ...
    train(
            ...
            train_dataloader_shuffling=train_dataloader_shuffling,
